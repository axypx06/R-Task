  ...
  return <>
   {data.length==0? <LinearProgress/>:null}
    ...
    <div style={{width:'100%',display:'flex',justifyContent:'space-between',marginTop:'20px'}}>
        ...
        <Autocomplete   id="free-solo-2-demo" sx={{width:100,marginTop:'20px',marginLeft:25, marginBottom:7,borderRadius:'10px',backgroundColor:'#E3D485'}}  disableClearable options={serachData} renderInput={params => ( <TextField {...params} label="Search Transactions" variant="outlined" /> )} getOptionLabel={option => option.title} style={{ width: 270 }} value={selectedTeam}  onChange={(_event, newTeam) => { setSelectedTeam(newTeam); }}  />
      </div>
      <div>
        <FormControl sx={{width:'200px' ,marginLeft:125,marginTop:'20px',borderRadius:'10px',backgroundColor:'#E3D485 '}} >
    ...
    <Paper sx={{ width: '80%', overflow: 'hidden', borderRadius: '20px', marginTop: '10px', margin: '0 auto' }}>
  <TableContainer component={Paper} sx={{ maxHeight: 440 }}>
    <Table sx={{ minWidth: 100 ,padding: '20px',backgroundColor:'#E3D485 '}}  stickyHeader aria-label="sticky table">
      <TableHead>
        <TableRow >
      ...
      <TableBody>
        {data.map((row) => (
          ...
          >
            ...
            <TableCell align="left"><img style={{width:"100px",height:"100px"}} src={row.image} alt={row.title} /></TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
<TablePagination
  ...
  sx={{ marginRight: 22, padding: '10px', color: 'black' }}
